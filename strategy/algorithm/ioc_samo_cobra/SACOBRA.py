...
def standardize_obj(x):
    mean = np.mean(x)
    std = np.std(x)
    # VENDORED, MODIFIED: guard std == 0, exactly as rescale_constr guards a
    # zero divider and for the same reason -- a constant column is reachable on
    # a rounded integer NAS space (a tie plateau, an all-invalid design, or an
    # all-infeasible one under the hard gate, whose objective rows are all
    # imputed with the same reference point). Upstream divides regardless and
    # poisons every surrogate downstream with nan.
    if std == 0:
        std = 1.0
    x = (x-mean)/std
    return x, mean, std
