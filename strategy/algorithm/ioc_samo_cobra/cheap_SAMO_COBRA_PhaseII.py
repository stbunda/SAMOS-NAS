...
# VENDORED, MODIFIED: visualiseParetoFront is not copied into this repo. It is
# a matplotlib/mplot3d live-plotting helper called from exactly one place, under
# `if cobra['plot']`, which a batch campaign never sets -- so the only thing the
# import bought was an unconditional matplotlib import on a headless node.
...
# VENDORED, MODIFIED: pygmo -> pymoo, see hypervolume.py for the rationale.
# pg.fast_non_dominated_sorting(F)[0][0] and
# NonDominatedSorting().do(F, only_non_dominated_front=True) are the same thing:
# the indices of the first (non-dominated) front.
from pymoo.util.nds.non_dominated_sorting import NonDominatedSorting

try:
    from scipy.optimize import _cobyla as cobyla  # removed in SciPy >= 1.10
except ImportError:
    cobyla = None  # unused; COBYLA is still available via minimize(method='COBYLA')
...
def batch_infill_criteria_score(x, batch, lower, upper, surrogateModels, bestPredictor, nObj, infillCriteria, FresPlogStandardizedStd, FresStandardizedStd, FresPlogStandardizedMean, FresStandardizedMean, paretoFrontier, ref, fnCheap, cheapObj):
    ...
    ndf0 = NonDominatedSorting().do(tempPF, only_non_dominated_front=True)
    pf_indicator = np.zeros(len(tempPF))==1
    pf_indicator[ndf0] = True
...
def cheap_SAMO_COBRA_PhaseII(cobra):
    ...
    # VENDORED, MODIFIED: the block below is now OPT-IN (cobra['save_results'],
    # default False -- see cheap_SAMO_COBRA_Init). Upstream ran it
    # unconditionally, which is wrong for this repo on two counts:
    #
    #   * it writes six CSVs and a JSON into ./batchresults/<name>/batch<N>/
    #     relative to the CURRENT WORKING DIRECTORY. On the cluster that is the
    #     per-task scratch dir (destroyed on exit); run locally from the repo
    #     root it would litter the repo. Callers here already persist
    #     everything they need through run_scenario.py's pkl.
    #   * <name> comes from ``str(fn).split(' ')[6]`` -- it parses the repr of
    #     the objective function object. That happens to yield a class name for
    #     a plain instance method, but IndexErrors outright on a repr with
    #     fewer than seven space-separated tokens (a functools.partial, a
    #     lambda, many bound methods), so it is a latent crash on the way OUT
    #     of an otherwise finished run.
    if not cobra.get('save_results', False):
        if pool:
            pool.close()
            pool.join()
        return cobra
